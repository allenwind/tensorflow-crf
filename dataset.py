...
# 直接复制自我的另外一个项目中的文件：
# https://github.com/allenwind/chinese-cut-word/blob/master/dataset.py
# 有些函数这里用不上
...
def load_icwb2_pku(file=_PKU, shuffle=True):
    with open(file, "r") as fp:
        text = fp.read()
...
def load_ctb6_cws(path=_CTB6, file="train.txt", shuffle=True):
    if not file.endswith(".txt"):
        file += ".txt"
    file = path + file
    # 复用load_icwb2_pku的加载方法
    return load_icwb2_pku(file, shuffle)

def load_random_sentences(words, size, maxwords=30):
    # 根据词频随机构造句子，用于数据增强
    ...
    sentences = []
    for i in range(size):
        sentence = np.random.choice(words, maxwords, p=p)
...
class DataGenerator:
    ...
    def __init__(self, sentences, tokenizer, batch_size, epochs=None):
    ...
    def __call__(self):
        for i in range(self.epochs):
            for sentence in self.sentences:
                x = self.tokenizer.transform_one("".join(sentence))
                y = compute_sbme_tags(sentence)
                yield x, y
...
class CharTokenizer:
    ...
    def save(self, file):
        with open(file, "w") as fp:
            json.dump(self.char2id, fp, indent=4, ensure_ascii=False)
